...
export default function AdminDashboard() {
  ...
  async function fetchPending() {
    ...
    try {
      ...
      toast.error(axiosMsg(err, 'Failed to load pending documents.'));
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleReview(doc: PendingDocument, status: ReviewStatus) {
    ...
    try {
      ...
      const label =
        status === 'APPROVED' ? `Document approved for ${doc.user.name}.` :
        status === 'REJECTED' ? `Document rejected.` :
                                `Document returned for revision.`;
      toast.success(label);
    } catch (err) {
      toast.error(axiosMsg(err, 'Review action failed. Please try again.'));
    } finally {
  ...
  async function handleLoadThematicStats() {
    ...
    try {
      ...
      toast.error(axiosMsg(err, 'Failed to load thematic stats.'));
    } finally {
  ...
  async function loadAppeals(force = false) {
    ...
    try {
      ...
      toast.error(axiosMsg(err, 'Failed to load appeals.'));
    } finally {
  ...
  async function handleSubmitReview(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      toast.success('Appeal decision submitted.');
    } catch (err) {
      toast.error(axiosMsg(err, 'Failed to submit review.'));
    } finally {
  ...
  return (
    ...
    {reviewingAppeal && (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
        <div className="bg-white rounded-xl shadow-xl w-full max-w-lg space-y-4 p-6 animate-modal-in">
      ...
      <div key={activeTab} className="animate-fade-in">
          ...
          <div className="space-y-5">
            ...
            {!showingSessionReport && (
              <>
                ...
                <div key={reportSubTab} className="animate-fade-in">
                            ...
                            <table className="min-w-full divide-y divide-gray-100 text-sm">
                              ...
                              <tbody className="divide-y divide-gray-100">
                                {thematicStats.chapters.map((ch, i) => (
                                  <tr key={ch.chapterId} className="hover:bg-gray-50 transition-colors">
        ...
        {activeTab === 'appeals' && (
          <div className="space-y-4">
            ...
            {appealsLoading && <SkeletonTable rows={4} cols={7} />}
